...
pub async fn get_block_info(
    ...
    now: u64,
    url: &str,
) -> Result<Vec<String>, BlockRequestError> {
    ...
    let raw_data: Value =
        serde_json::from_str(&res).map_err(BlockRequestError::JsonConversionError)?;
    let data = raw_data["result"].clone();
    let hash = data["hash"]
        .as_str()
        .ok_or(BlockRequestError::ConversionError("hash to str"))?;
    let validator = data["miner"]
        .as_str()
        .ok_or(BlockRequestError::ConversionError("miner address to str"))?;
    let txs = data["transactions"]
        .as_array()
        .ok_or(BlockRequestError::ConversionError("transactions to array"))?;
    ...
    texts.push(String::from(format!(
        "---transfer: {}, deployment: {}, execution: {}, blob: {}",
    ...
    // EIP-1559 Feature
    let target_diff =
        (100 as f64) * ((gas_used as f64) - (gas_target as f64)) / (gas_target as f64);
    let max_diff = (100 as f64) * (gas_used as f64) / (gas_max as f64);
    texts.push(String::from(format!(
        "---Gas target: {}, Gas total usage {}",
        format_generic(gas_target as u32),
        format_generic(gas_used as u32)
    )));
    texts.push(String::from(format!(
        "---Gas objective {:.2}% from target, {:.2}% of maximum",
        target_diff, max_diff
    )));
    if target_diff < 0. {
    ...
    texts.push(String::from(format!(
        "---Gas usage: min={}, max={}, avg={}",
        format_generic(min_gas as u32),
        format_generic(max_gas as u32),
        format_generic(avg_gas as u32)
    )));
    texts.push(String::from(format!(
        "---Gas price: min={:.2} Gwei, max={:.2} Gwei, avg={:.2} Gwei",
    ...
    texts.push(String::from(format!(
        "---Base fee: {:.2} gwei",
        (base_fee as f64) * 1e-9
    )));
    texts.push(String::from(format!(
        "---Priority fee: min={:.2} Gwei, max={:.2} Gwei, avg {:.2} gwei",
        (min_gas_price as f64) * 1e-9 - (base_fee as f64) * 1e-9,
    ...
    if data.get("blobGasUsed").is_some() {
        ...
        texts.push(String::from(format!(
            "---Blob gas used: {}",
            format_generic(blob_gas_used as u32)
        )));
        texts.push(String::from(format!(
            "---Excess blob gas: {}",
            format_generic(excess_blob_gas as u32)
        )));
        let blob_gas_target: u64 = 393216;
        let blob_gas_max: u64 = 786432;
        let blob_target_diff = (100 as f64) * ((blob_gas_used as f64) - (blob_gas_target as f64))
            / (blob_gas_target as f64);
        let blob_max_diff = (100 as f64) * (blob_gas_used as f64) / (blob_gas_max as f64);
        texts.push(String::from(format!(
            "---Blob gas target: {}, Blob gas total usage {}, Blob excess gas {}",
            format_generic(blob_gas_target as u32),
            format_generic(blob_gas_used as u32),
            format_generic(excess_blob_gas as u32)
        )));
        texts.push(String::from(format!(
            "---Blob gas objective {:.2}% from target, {:.2}% of maximum",
            blob_target_diff, blob_max_diff
        )));
